...
const TeamCard = ({
  ...
  return (
    <>
      <div className="bg-[#2B1954] w-full rounded-lg text-white flex flex-col items-center justify-center m-3 p-5 shadow-sm shadow-amber-500">
        <Image src={image_url} alt="Team Member" width={200} height={200} />
        <h2 className="text-2xl font-semibold py-2">{name}</h2>
        <h3 className="text-2xl font-semibold py-2">{role}</h3>
        <p>{tech_stach}</p>
        <Link
